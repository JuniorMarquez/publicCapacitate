...
app.controller('SolicitudesCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster','$timeout',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster,$timeout) {
 $scope.nivel=MyService.data.nivel;
  ...
  $scope.consultarEnProceso = function(){
       $http.get('http://54.202.62.62:1346/solicitud/?estado=En proceso').then(function (resp) {   
  ...
  $scope.consultarEnProceso();
   $scope.consultarPendientes = function(){
    $http.get('http://54.202.62.62:1346/solicitud/?estado=Pendiente').then(function (resp) {
      $scope.solicitudes = resp.data.results;
      $scope.detallesSolicitud = null;
      $scope.lateral = false;   
    });
   ...
   $scope.consultarFinalizadas = function(){
    $http.get('http://54.202.62.62:1346/solicitud/?estado=Finalizada').then(function (resp) {
    $scope.solicitudes = resp.data.results;
    $scope.detallesSolicitud = null;
     $scope.lateral = true; 
    });
  ...
  $scope.deleteSolicitud = function(item){
    $http.delete('http://54.202.62.62:1346/solicitud/'+item.id , item)
    $scope.solicitudes.splice($scope.solicitudes.indexOf(item), 1);
  };
  ...
  $scope.selectSolicitud = function(item){ 
    ...
    $http.get('http://54.202.62.62:1346/mensaje/?idSolicitud='+item.id).then(function (resp) {
  ...
  $scope.selectItem = function(item){    
    ...
    $http.get('http://54.202.62.62:1346/consultor/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
  ...
  $scope.deleteItem = function(item){
    $http.delete('http://54.202.62.62:1346/consultor/'+item.id , item)
  ...
  $scope.deleteconsultor = function(consultor){
    $http.delete('http://54.202.62.62:1346/consultor/'+consultor.id , consultor)
  ...
  $scope.createItem = function(){
    ...
    $http.get('http://54.202.62.62:1346/solicitud/').then(function (resp) {
    $scope.solicitudes = resp.data.results;
    }); 
  };
  ...
  $scope.solicitud = function(item){
    ...
    if (MyService.data.idenSolicitud){
      $http.put('http://54.202.62.62:1346/solicitud/'+MyService.data.idenSolicitud, solicitud)
    }
    else {
      $http.post('http://54.202.62.62:1346/solicitud/', solicitud)
    }
    $scope.items = null;
    $scope.item = null;
  ...
  $scope.doneEditingConsultor = function(item){
    ...
    if (MyService.data.idenConsultor){
      $scope.pop4();
      $http.put('http://54.202.62.62:1346/consultor/'+MyService.data.idenConsultor , consultorAct)
    }
    else {
      $scope.pop3();;
      $http.post('http://54.202.62.62:1346/consultor/', consultorAct)
    }
    $http.get('http://54.202.62.62:1346/solicitud/').then(function (resp) {
      $scope.solicitudes = resp.data.results;
    });
    $http.get('http://54.202.62.62:1346/consultor/').then(function (resp) {
