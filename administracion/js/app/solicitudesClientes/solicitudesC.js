...
$scope.lateral = false;
  ...
  $scope.consultarSolicitudes= function(){
       $http.get('http://54.202.62.62:1346/solicitud/?idUsuario='+MyService.data.idUsuario).then(function (resp) {   
...
$scope.consultarFinalizadas = function(){
  ...
  $scope.consultarMensajesDeNuevo=function(item){
     $http.get('http://54.202.62.62:1346/mensaje/?idSolicitud='+item).then(function (resp) {
      $scope.items = resp.data.results;
      // $scope.item = null;  
    });
  };
